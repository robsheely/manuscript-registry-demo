...
export const DealsKanban = () => {
      ...
      repo(Deal)
        ...
        .then((deals) => {
          ...
          for (const d of deals) {
            if (newDealsForStage[d.stage]) newDealsForStage[d.stage].push(d)
          }
  ...
  const onDragEnd: OnDragEndResponder = async ({
    ...
    if (!dealsForStage) return

    const stage = destination.droppableId
    const onDealId =
      dealsForStage[destination.droppableId][destination.index]?.id
    Deal.organizeLists({
      dealId: draggableId,
      stage,
      onDealId,
    ...
    <DragDropContext onDragEnd={onDragEnd}>
      <Box display="flex" sx={{ overflowX: 'auto' }}>
        {DealStages.map((stage) => (
          <DealColumn stage={stage} deals={dealsForStage[stage]} key={stage} />
        ))}
      </Box>
    </DragDropContext>
  )
