...
export const AdminPage = () => {
  ...
  const HeaderCell: React.FC<{ field: FieldMetadata }> = ({ field }) => {
    ...
    const handleClick = () => {
      setOrderBy({
        [field.key]:
          current === 'asc' ? 'desc' : current === undefined ? 'asc' : undefined
      })
    }
    return (
      <HeaderCellStyle onClick={handleClick}>
        {field.caption}{' '}
  ...
  return (
    <div>
      ...
      <TheTable style={{ maxHeight: 200 }}>
        ...
        {items?.items.map((x) => (
          <Line key={x.id}>
            ...
            <LineCell style={{ position: 'sticky', right: 0, width: 'auto' }}>
              <button>test</button>
            </LineCell>
          </Line>
        ))}
      ...
      <button
        ...
        onClick={() => items?.nextPage().then(setItems)}
      >
        Next Page
      </button>
    </div>
